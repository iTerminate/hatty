...
async def test_i_v_advances_to_device_view_and_sends_the_entitys_device_id(
    ...
    async with app.run_test() as pilot:
        ...
        # This view widens the event-type query, not the entity set — the sibling
        # kitchen_light only appears in the next ("device entities") view below.
...
async def test_i_v_v_reaches_device_entities_view(make_app, sample_entities, sample_registry):
    app = make_app(entities=sample_entities, config_data=NO_LIST_CONFIG, registry=sample_registry)
    async with app.run_test() as pilot:
        await pilot.pause()
        table = app.query_one("EntitiesTable")
...
async def test_device_log_live_update_from_sibling(make_app, sample_entities, sample_registry):
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.press("v")
        await pilot.pause()
        await pilot.press("v")  # device entities view: siblings are in scope
        await pilot.pause()
...
async def test_v_scopes_to_graphed_entity_and_wraps_after_three_views(make_app, sample_entities, sample_registry):
    ...
    (graph-based) scope offers no cursor views, so `v` wraps after 3 presses,
    unlike the 5-view table-base cycle (issue #27)."""
    config = {
