...
(issue #69), then every sibling entity on those devices, then narrows to the
cursor's entity and its device, then wraps (issue #27)."""
...
async def test_v_adds_the_lists_device_ids_without_expanding_siblings(make_app, sample_entities, sample_registry):
    ...
    # entity set — sibling expansion is the next view.
...
async def test_v_v_expands_all_devices_and_siblings_in_list(make_app, sample_entities, sample_registry):
    config = _list_config(["light.living_room_lamp"])
    app = make_app(entities=sample_entities, config_data=config, registry=sample_registry)
    async with app.run_test() as pilot:
        await pilot.pause()
...
async def test_v_v_title_shows_device_entities_log_and_device_count(make_app, sample_entities, sample_registry):
    config = _list_config(["light.living_room_lamp", "sensor.temperature"])
    app = make_app(entities=sample_entities, config_data=config, registry=sample_registry)
    async with app.run_test() as pilot:
        await pilot.pause()
...
async def test_v_cycles_the_list_base_through_five_scopes_and_wraps(make_app, sample_entities, sample_registry):
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.press("v")  # 3: every sibling entity on those devices
        await pilot.pause()
        assert app._log_entity_ids == {
            "light.living_room_lamp",
            "light.kitchen_light",
        ...
        await pilot.press("v")  # 4: the cursor entity alone
        await pilot.pause()
        ...
        await pilot.press("v")  # 5: the cursor entity's device
        await pilot.pause()
